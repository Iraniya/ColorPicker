...
class TopSliderLayer: CALayer {
    ...
    override func draw(in ctx: CGContext) {
        if let colorBallView = colorBallView {
            ...
            //ctx.setStrokeColor(colorBallView.ttrackColor.cgColor)
            ...
            //ctx.setStrokeColor((colorBallView.tProgressColor.cgColor))
            ctx.move(to: CGPoint(x: 0, y: radius))
            ...
            //ctx.setFillColor(colorBallView.tProgressColor.cgColor)
            ctx.fillPath()
        }
    }
}
